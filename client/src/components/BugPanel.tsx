...
export const BugPanel = ({currentUserId, reporterId, reportId }: BugPanelProps) => {
  ...
  const [reporter, setReporter] = useState<UserDetail | null>(null);
  ...
  useEffect(() => {
    const getUserData = async () => {
      ...
      if (!reporterId && reporterId !== -1) {
        const userResp = await getUsersById(reporterId);
  ...
  useEffect(() => {
    const getBugReport = async () => {
      if (!reportId && reportId !== -1) {
        const reportResp = await getBugReportById(reporterId);
      ...
      <div className={'bug-panel'}>
        <div className="bug-reporter" key={reporterId}>
            <img
                ...
                src={reporter?.profileImage ? reporter.profileImage : ''/*? reporter.profileImage : `${profilePicture}`*/}
                ...
                <Popup>
                  ...
                  <PopupContent>
                    <div className="small-popup" id="report-popup">
                      ...
                      <p>Here is the context behind {reporter?.firstName ?? "User"}'s report:</p>
