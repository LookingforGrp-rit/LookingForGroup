...
const ChooseSkills: React.FC<ChooseSkillsProps> = ({
	...
	//wait... only if? you can't pick more than 5 at a time? why?
	const handleSkillSelect = (
		skill: React.MouseEvent<HTMLButtonElement, MouseEvent>
	) => {
		// get the skill that was selected
		const selected = (skill.target as Element).innerHTML;
		//i want the skillId not the skill itself for creation
		//i guess when it comes time to do that i can just extract them from the label? okay? but i'd then need to make another call
		//smth i *could* do is have the
		const skillId = skills?.find((s) => s.label === selected)?.skillId;

		if (skillId) {
			// if the skill is already selected, remove it from the selectedSkills array
			if (selectedSkills.includes(selected)) {
				setSelectedSkills(
					selectedSkills.filter((prof) => prof !== selected)
				);
				setSelectedSkillIds(
					selectedSkillIds.filter((prof) => prof !== skillId)
				);
			}
				...
				<div className="ChooseSkills">
					...
					{mode === "signup" ? (
						...
						<>
							...
							<p>
								Add skills using the search bar, then select 5
								skills to be highlighted on your page
							</p>
						</>
					)}
