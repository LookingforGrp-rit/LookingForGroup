...
const deleteTag = async (request: AuthenticatedRequest, response: Response): Promise<void> => {
  const res: ApiResponse = { status: 0 };
  const tagId = parseInt(request.params.id as string);
  const result = await deleteTagFromSiteService(tagId);
