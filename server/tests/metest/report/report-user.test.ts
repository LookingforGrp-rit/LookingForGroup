...
describe('reportUserService', async () => {
  ...
  it('returns OK when it creates a report', async () => {
    vi.mocked(prisma.reportUser.findFirst).mockResolvedValue(null);

    const result = await reportUserService(1, 2, 'test report');

    expect(prisma.reportUser.create).toHaveBeenCalledWith({
      data: {
        reporterId: 1,
        reportedId: 2,
        reportText: 'test report',
  ...
  it('returns CONFLICT if there already is report', async () => {
    vi.mocked(prisma.reportUser.findFirst).mockResolvedValue(prismaReport);
  ...
  it('returns INTERNAL_ERROR if prisma throws', async () => {
    vi.mocked(prisma.reportUser.findFirst).mockRejectedValue(new Error('womp womp'));
