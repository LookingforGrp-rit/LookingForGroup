...
describe('getThumbnailService', async () => {
  ...
  it('returns thumbnail if possible', async () => {
    ...
    const result = await getThumbnailService(100);
  ...
  it("returns NOT_FOUND if the project doesn't exist", async () => {
  ...
  it("returns NOT_FOUND if the thumbnail doesn't exist", async () => {
    vi.mocked(prisma.projects.findUnique).mockResolvedValue({ projectId: 100 } as Projects);
    vi.mocked(prisma.projectImages.findUnique).mockResolvedValue(thumbImage);
    vi.mocked(transformProjectImage).mockReturnValue(transformedThumb);
    const result = await getThumbnailService(100);
