...
vi.mock('#services/datasets/get-socials.ts', () => ({
  getSocialsService: vi.fn(),
}));
...
describe('Get socials', () => {
  ...
  test('should be using the mocked service', () => {
    expect(getSocialsService).toBe(vi.mocked(getSocialsService));
  });

  test('should normally return 200', async () => {
    vi.mocked(getSocialsService).mockClear();
    vi.mocked(res.json).mockClear();
    vi.mocked(res.status).mockClear();
    vi.mocked(getSocialsService).mockResolvedValue([]);
    ...
    await getSocialsController(req, res);

    expect(getSocialsService).toHaveBeenCalledOnce();
  ...
  test('should return 500 if the service returns INTERNAL_ERROR', async () => {
    ...
    vi.mocked(res.json).mockClear();
    vi.mocked(res.status).mockClear();
    vi.mocked(getSocialsService).mockResolvedValue('INTERNAL_ERROR');
