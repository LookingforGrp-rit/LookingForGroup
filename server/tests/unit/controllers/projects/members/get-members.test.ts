import { describe, expect, test, vi, beforeEach, afterEach } from 'vitest';
import getMembersController from '#controllers/projects/members/get-members.ts';
import getMembersService from '#services/projects/members/get-members.ts';
import { blankIdRequest, blankResponse } from '#tests/resources/blanks/extra.ts';
import { blankProjectMember } from '#tests/resources/blanks/projects.ts';

vi.mock('#services/projects/members/get-members.ts');

//dummy req
const req = blankIdRequest;

//dummy resp
const res = blankResponse;

describe('getMembers', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  afterEach(() => {
    vi.restoreAllMocks();
  });

  //there's something wrong with the service, should return 500
  test('Must return 500 when the service errors', async () => {
    vi.mocked(getMembersService).mockResolvedValue('INTERNAL_ERROR');
    expect(getMembersService).toBe(vi.mocked(getMembersService));
    const resBody = {
      status: 500,
      error: 'Internal Server Error',
      data: null,
    };

    await getMembersController(req, res);
    expect(getMembersService).toHaveBeenCalledOnce();
    expect(res.status).toHaveBeenCalledWith(500);
    expect(res.json).toHaveBeenCalledWith(resBody);
  });

  //members could not be found, should return 404
  test('Must return 404 when the members could not be found', async () => {
    vi.mocked(getMembersService).mockResolvedValue('NOT_FOUND');
    expect(getMembersService).toBe(vi.mocked(getMembersService));
    const resBody = {
      status: 404,
      error: 'Members not found',
      data: null,
    };

    await getMembersController(req, res);
    expect(getMembersService).toHaveBeenCalledOnce();
    expect(res.status).toHaveBeenCalledWith(404);
    expect(res.json).toHaveBeenCalledWith(resBody);
  });
  //everything's good, return 200
  test('Must return 200 when the members were retrieved successfully', async () => {
    vi.mocked(getMembersService).mockResolvedValue([blankProjectMember]);
    expect(getMembersService).toBe(vi.mocked(getMembersService));
    const resBody = {
      status: 200,
      error: null,
      data: [blankProjectMember],
    };

    await getMembersController(req, res);
    expect(getMembersService).toHaveBeenCalledOnce();
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(resBody);
  });
});
