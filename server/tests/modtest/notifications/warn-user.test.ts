...
describe('warnUserService', () => {
  ...
  it('creates a warning notification and sends the warning email', async () => {
    vi.mocked(prisma.users.findUnique).mockResolvedValue(prismaUser);

    const result = await warnUserService(request);
